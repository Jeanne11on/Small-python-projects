...
def getBirthdays(numberOfBirthdays):
    ...
    for i in range(numberOfBirthdays):
        "Year unimportant here, as long as all birthdays have the same year."
...
def getMatch(birthdays):
    ...
    if len(birthdays)==len(set(birthdays)):
        return None # if we get there, then all birthdays are unique
    
    # Here we compare each birthday to every other birthday:
...
while True: #Keep asking until the user enters a valid amount. 
    ...
    if response.isdecimal() and (0 < int(response) <= 100):
        ...
        break # if we get here, then the user has entered a valid amount
print()
